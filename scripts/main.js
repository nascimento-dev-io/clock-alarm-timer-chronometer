...
function getPage(){
  ...
  fetch(`http://127.0.0.1:5500/pages/${page}.html`)
   .then((res) => res.text())
...
function clock(){  
  ...
  const hora = `${new Date().getHours() < 10 ? "0"+new Date().getHours() : new Date().getHours()} : ${new Date().getMinutes() < 10 ? "0"+new Date().getMinutes() : new Date().getMinutes()} : ${new Date().getSeconds() < 10 ? "0"+new Date().getSeconds() : new Date().getSeconds()}`
